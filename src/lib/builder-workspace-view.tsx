...
export function BuilderView({
  ...
  useEffect(() => {
    ...
    const fromUrl = (() => {
      try {
        return new URL(window.location.href).searchParams.get("c")?.trim() || null;
      } catch {
        return null;
      }
    })();
  ...
  // UI is loaded from the original workspace shell via dynamic import pattern below
  // For a complete UI, we render a compact shell that covers idle + main states.
    ...
    return (
      <div className="relative flex min-h-[100dvh] w-full flex-col items-center justify-center bg-canvas px-4">
        ...
        <p className="mt-2 max-w-md text-center text-[14px] text-ink-3">
          Your chat, files, and preview are saved in this browser and restore after refresh.
        </p>
  ...
  return (
    <div className="relative flex h-[100dvh] min-h-0 w-full flex-col bg-canvas lg:flex-row">
      <aside className="flex min-h-0 w-full flex-col border-b border-line lg:w-[380px] lg:border-b-0 lg:border-r">
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-auto p-3">
          ...
          {tasks.map((t) => (
            <ProcessRow key={t.id} kind={t.kind === "write" ? "file" : "think"} label={t.label} active={t.state === "run"} />
          ))}
          {questionsOpen && questions.length ? (
            <QuestionBox
              ...
              onSubmit={() => void plan_(idea, answers)}
            />
          ) : null}
          {plan && phase === "review" ? (
            <PlanPanel
