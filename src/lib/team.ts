...
export async function deleteTeam(confirmName: string) {
  ...
  const clean = confirmName.trim().replace(/\s+/g, " ");
  if (clean !== team.name) throw new Error("TEAM_NAME_CONFIRMATION");

  await batch(
    [
      { sql: "DELETE FROM team_invites WHERE team_id = ?", args: [team.id] },
      { sql: "DELETE FROM team_projects WHERE team_id = ?", args: [team.id] },
      { sql: "DELETE FROM team_members WHERE team_id = ?", args: [team.id] },
      { sql: "DELETE FROM teams WHERE id = ?", args: [team.id] },
    ],
  );
  return { ok: true };
}
