...
function cleanProperties(input?: Properties): Properties {
  ...
  for (const [rawKey, rawValue] of Object.entries(input).slice(0, 20)) {
    ...
    if (!key) continue;
    if (typeof rawValue === "string") out[key] = rawValue.slice(0, 120);
 ...
 * Never put prompts, generated output, email addresses, secrets, filenames,
 * URLs chosen by users, or raw errors into properties.
 */
...
export async function trackEventOncePerUser(
  input: TrackInput & { userId: string },
): Promise<void> {
  const event = cleanEvent(input.event);
  ...
  try {
    ...
    const existing = await one(
      `SELECT id FROM analytics_events WHERE user_id = ? AND event = ? LIMIT 1`,
      [input.userId.slice(0, 80), event],
    );
    if (existing) return;
    await trackEvent(input);
  } catch (error) {
    console.error(
      "analytics: first-event write failed",
      error instanceof Error ? error.message : String(error),
    );
  }
}
...
export async function analyticsSummary(days = 30) {
  ...
  const [events, daily, funnel] = await Promise.all([
    all(
      `SELECT event,
              COUNT(*) AS count,
    ...
    all(
      `SELECT event,
        ...
        WHERE created_at >= ?
          AND user_id <> ''
          AND event IN ('signup_completed','first_prompt','builder_project_created','checkout_started','checkout_created','site_published')
        GROUP BY event`,
      [since],
    ),
  ]);
