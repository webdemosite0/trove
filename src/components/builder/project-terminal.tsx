...
export function ProjectTerminal({
  ...
  // Auto-start cloud sandbox + Preview once files exist (React/Vite need it)
  const autoBooted = useRef(false);
  useEffect(() => {
    if (autoBooted.current || !files.length || sandboxId) return;
    autoBooted.current = true;
    void bootSandbox();
  }, [files.length, sandboxId]);
  ...
  async function bootSandbox() {
    ...
    print("  (npm install + vite may take 1–2 minutes)");
    try {
