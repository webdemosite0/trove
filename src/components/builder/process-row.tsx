...
function FileIcon({ className }: { className?: string }) {
  return (
    <svg className={className} width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden>
      <path d="M4 2.5h5.5L12.5 5.5V13.5a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1z" stroke="currentColor" strokeWidth="1.25" />
...
function SparkIcon({ className }: { className?: string }) {
  return (
    <svg className={className} width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden>
      <path d="M8 1.5 L9.2 6.2 L14 7.5 L9.2 8.8 L8 13.5 L6.8 8.8 L2 7.5 L6.8 6.2 Z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" />
    </svg>
  );
}
...
function Chevron({ open }: { open: boolean }) {
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden className={cn("shrink-0 text-ink-4 transition-transform duration-200", open && "rotate-180")}>
      <path d="M4 6 L8 10 L12 6" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
export function ProcessRow({
  kind, label, active = false, connectorId, index, className,
}: {
  ...
  return (
    <div className={cn("group flex items-start gap-2 py-1 text-[13px] leading-snug", active ? "text-ink-2" : "text-ink-3", className)}>
      {typeof index === "number" ? (
        ...
        <span className={cn("mt-px grid size-[18px] shrink-0 place-items-center", active ? "text-ink-2" : "text-ink-4")}>
          {kind === "connect" && connectorId ? <ServiceMark id={connectorId} size={14} /> : meta.icon === "term" ? <TermIcon /> : meta.icon === "file" ? <FileIcon /> : meta.icon === "check" ? <CheckIcon /> : <SparkIcon />}
        </span>
      )}
      {typeof index === "number" ? <span className="mt-px shrink-0 text-ink-4" aria-hidden>→</span> : null}
      <span className="min-w-0 flex-1">
        {kind === "connect" ? (
          <><span className="text-ink-4">Used </span><span className="font-medium text-ink-2">{connectorId ? `${connectorId.charAt(0).toUpperCase()}${connectorId.slice(1).replace(/-/g, " ")} Connector` : "Connector"}</span></>
        ) : showPrefix ? (
          <><span className="text-ink-4">{meta.prefix}</span>{label ? <>{" "}<span className={cn("text-ink-2", active && "text-ink")}>{label}</span></> : null}</>
        ) : (
...
export function WorkingTimer({ secs, className }: { secs: number; className?: string }) {
  return (
    <div className={cn("flex items-center gap-2.5 py-[3px] text-[13px] text-ink-3", className)}>
      <span className="grid size-[18px] place-items-center text-accent"><SparkIcon className="animate-pulse" /></span>
      <span>Working for <span className="tabular-nums text-ink-2">{formatSecs(secs)}</span></span>
    </div>
  );
}

export type ThinkingStep = { id: string; kind: ProcessKind; label: string; active?: boolean; connectorId?: string };

/** Collapsed by default — click "Thinking" to expand numbered steps. */
export function ThinkingTrace({ steps, running, className }: { steps: ThinkingStep[]; running?: boolean; className?: string }) {
  ...
  const summary = running
    ? activeLabel ? `Thinking · ${activeLabel}` : "Thinking"
    : steps.length === 1 ? "Thought" : `Thought · ${steps.length} steps`;
  return (
    <div className={cn("rounded-xl border border-line/80 bg-canvas/40 px-2.5 py-1.5", className)}>
      <button type="button" onClick={() => setOpen((v) => !v)} className="flex w-full items-center gap-2 rounded-lg px-1 py-1 text-left transition-colors hover:bg-hover/50" aria-expanded={open}>
        <span className="grid size-5 shrink-0 place-items-center text-accent">
          {running ? <span className="block size-3.5 animate-spin rounded-full border-[1.5px] border-accent border-t-transparent" /> : <SparkIcon className="text-ink-3" />}
        </span>
      ...
      {open ? (
        <div className="mt-1 max-h-[min(40vh,280px)] space-y-0.5 overflow-y-auto border-t border-line/70 pt-1.5 pl-1">
          {steps.map((s, i) => (
            <ProcessRow key={s.id} kind={s.kind} label={s.label} active={s.active} connectorId={s.connectorId} index={i + 1} />
          ))}
          <div ref={end} />
        </div>
      ) : null}
...
function parseChatBlocks(raw: string): ChatBlock[] {
  ...
  while (i < lines.length) {
    ...
    if (/^#{1,2}\s+/.test(trimmed)) {
      out.push({ type: trimmed.startsWith("##") ? "h2" : "h1", text: trimmed.replace(/^#{1,2}\s+/, "") });
      i += 1; continue;
    }
    ...
    while (i < lines.length) {
      ...
      if (!t || /^#{1,2}\s+/.test(t) || /^\d+[.)]\s+/.test(t) || /^[-*•]\s+/.test(t) || /^(→|->|=>|—)\s+/.test(t)) break;
      parts.push(t); i += 1;
    }
...
function renderInline(text: string): ReactNode {
  ...
  while ((m = re.exec(text))) {
    ...
    const token = m[0];
    if (token.startsWith("**")) nodes.push(<strong key={k++} className="font-semibold text-ink">{token.slice(2, -2)}</strong>);
    else nodes.push(<code key={k++} className="rounded bg-sunk px-1 py-0.5 font-mono text-[12px] text-ink">{token.slice(1, -1)}</code>);
