...
export function Faq() {
  return (
    <section className="mx-auto max-w-[1140px] px-5 py-20 lg:px-8 lg:py-28">
      ...
      <div className="mx-auto mt-12 max-w-[720px] divide-y divide-zinc-200/90 rounded-2xl border border-zinc-200/90 bg-white/70 dark:divide-line dark:border-line dark:bg-raised/70 px-4 shadow-sm backdrop-blur-sm sm:px-6">
        {QA.map((item) => (
          <details key={item.q} className="group py-1">
            <summary
              className={
                "flex cursor-pointer list-none items-center gap-4 py-4 text-[15px] font-medium text-zinc-900 transition-colors dark:text-ink hover:text-violet-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/40"
              }
            >
              <span className="flex-1">{item.q}</span>
              <FiPlus
                size={17}
                className="shrink-0 text-zinc-400 dark:text-ink-4 transition-transform duration-200 group-open:rotate-45"
              />
            </summary>
            <p className="pb-5 pr-10 text-[14.5px] leading-relaxed text-zinc-600 dark:text-ink-3">{item.a}</p>
          </details>
        ))}
      </div>
    </section>
