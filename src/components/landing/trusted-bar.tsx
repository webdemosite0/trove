    ...
    <section className="relative px-4 py-8 sm:px-6 sm:py-12">
      <Reveal>
        <div className="mx-auto flex w-fit max-w-full items-center gap-2 overflow-x-auto rounded-full border border-black/[0.09] bg-white/90 px-2.5 py-2 shadow-[0_12px_35px_-22px_rgba(15,23,42,.42)] backdrop-blur-xl scrollbar-none sm:gap-2.5 sm:px-3">
          <span className="shrink-0 pl-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-zinc-500 sm:text-[12px]">
            Connect your tools
          </span>
          <span className="h-5 w-px shrink-0 bg-zinc-200" />
          {APPS.map((app, index) => (
            <Reveal key={app.id} delay={index * 24} as="span">
          ...
          <Link
            href="/integrations"
            className="ml-1 inline-flex size-8 shrink-0 items-center justify-center rounded-full bg-zinc-950 text-white transition hover:-translate-y-0.5 hover:bg-black"
      ...
      <Reveal delay={90}>
        <p className="mx-auto mt-3 max-w-[52ch] text-center text-[11px] leading-5 text-zinc-500 sm:text-[12px]">
          Bring your files, context, and favorite apps into one workspace.
        </p>
      </Reveal>
      ...
      {shownModels.length ? (
        <div className="mx-auto mt-8 max-w-[900px] sm:mt-10">
          <Reveal>
            <p className="text-center text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500 sm:text-[11px]">
              Choose your model in Chat
            </p>
          </Reveal>
