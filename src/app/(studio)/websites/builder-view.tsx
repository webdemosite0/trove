...
export function BuilderView({ mobile = false, draft = "" }: { mobile?: boolean; draft?: string }) {
    ...
    void (async () => {
      ...
      try {
        ...
        if (res.ok && data?.previewUrl) {
          ...
          log("Set E2B_API_KEY for cloud Preview URL", "warn");
        }
      } catch { /* local preview only */ }
      finally { setSandboxBooting(false); }
    })();
    try {
  ...
  const edit = useCallback(async (text: string, attach?: Attachment[]) => {
    ...
    setError(null); setPhase("building"); log(`edit — ${text.slice(0, 60)}`);
    try {
      const current = await runStep(
        { id: `edit-${Date.now()}`, title: text.slice(0, 40), detail: `Apply: ${text}. Keep locked brand answers.`, skills: [], files: [] },
        plan ? styleBrief(plan) : "Keep the existing visual language.",
        files,
        { index: 0, total: 1 },
        attach,
      );
        ...
        <div className="mx-auto flex w-full max-w-[760px] flex-1 flex-col justify-center px-5 py-16">
          <div className="mb-8 text-center">
            ...
            <p className="mx-auto mt-2 max-w-[48ch] text-[15px] text-ink-3">
              Plans, builds, and opens Preview for Static, React, Node, and Python.
            </p>
          </div>
          ...
          <div className="mt-3 flex justify-center gap-1.5">
            {([{ id: "quick" as const, label: "Quick" }, { id: "deep" as const, label: "Deep Build" }]).map((m) => (
              <button key={m.id} type="button" onClick={() => setDepth(m.id)} className={cn("rounded-full border px-3 py-1.5 text-[12.5px]", depth === m.id ? "border-accent bg-accent/10 text-ink" : "border-line text-ink-3")}>
                {m.label}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="flex h-[100dvh] max-h-[100dvh] min-h-0 flex-1 flex-col overflow-hidden">
      <header className="flex shrink-0 items-center gap-2 border-b border-line bg-rail/70 px-3 py-2">
        ...
        {busy ? <span className="text-[11px] text-accent">{phase === "planning" ? "Planning" : "Building"}</span> : null}
            ...
            <div className="flex min-h-0 flex-1 flex-col bg-sunk">
              <div className="flex shrink-0 items-center gap-2 border-b border-line px-3 py-2">
                ...
                {(Object.keys(DEVICE) as (keyof typeof DEVICE)[]).map((d) => {
                  const D = DEVICE[d];
                  return (
                    <button key={d} type="button" onClick={() => setDevice(d)} className={cn("grid size-7 place-items-center rounded", device === d ? "bg-hover text-ink" : "text-ink-4")} aria-label={D.label}>
                      <D.icon size={14} />
                    </button>
                  );
                })}
              ...
              <div className="flex min-h-0 flex-1 items-start justify-center overflow-auto p-3">
                {preview || sandboxUrl ? (
                  <div className="flex h-full min-h-[420px] flex-col overflow-hidden rounded-[12px] border border-line bg-[#1a1a1c]" style={{ width: DEVICE[device].w, maxWidth: "100%" }}>
                    <div className="flex h-9 shrink-0 items-center gap-2 border-b border-white/8 bg-[#222] px-3">
                      ...
                      {sandboxBooting ? <span className="ml-auto text-[11px] text-accent">booting…</span> : sandboxUrl ? <span className="ml-auto text-[11px] text-positive">live</span> : <span className="ml-auto text-[11px] text-white/30">local</span>}
                    </div>
                  ...
                  <div className="m-auto max-w-[40ch] text-center">
                    ...
                    <p className="mt-2 text-[13px] text-ink-4">
                      {busy ? "Building…" : files.length ? `Open Console and run sandbox for ${target.label}, or wait for HTML Preview.` : "Preview appears when files are ready."}
                    </p>
                    {target.serves ? <p className="mt-2 font-mono text-[12px] text-ink-3">{target.serves}</p> : null}
                    <button type="button" className="mt-3 rounded-full border border-line px-3 py-1.5 text-[12.5px]" onClick={() => setPane("console")}>Open Console</button>
                  </div>
                )}
              </div>
            </div>
