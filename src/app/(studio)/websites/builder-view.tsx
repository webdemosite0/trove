      ...
      <div className="relative flex min-h-screen flex-col">
        ...
        <div className="nx-in mx-auto flex w-full max-w-[760px] flex-1 flex-col justify-center px-5 py-16">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {TARGET_LIST.map((t, i) => (
              <button
                ...
                className={cn(
                  "nx-in rounded-[18px] border p-3.5 text-left transition duration-200 hover:-translate-y-0.5",
                  targetId === t.id
                    ? "border-accent/60 bg-accent/[0.07] shadow-[0_16px_50px_-30px_rgba(124,92,255,.5)]"
          ...
          <div className="mt-3 flex items-center justify-center gap-1.5">
            {([{ id: "quick" as const, label: "Quick", icon: FiZap, hint: "3-4 steps" }, { id: "deep" as const, label: "Deep Build", icon: FiLayers, hint: "5-8 steps" }]).map((m) => (
              <button
                ...
                className={cn(
                  "flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[12.5px] transition",
                  depth === m.id ? "border-accent bg-accent/10 text-ink" : "border-line text-ink-3 hover:bg-hover hover:text-ink-2",
                )}
              >
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-2.5">
            {IDEAS.map((e, i) => (
              <button key={e} onClick={() => ask(e)} className="chip group nx-in" style={{ animationDelay: `${80 + i * 50}ms`, animationFillMode: "backwards" }}>
                {e}
              </button>
            ))}
          </div>
    ...
    <div className="flex h-screen flex-col">
          ...
          {PANES.map((p) => (
            ...
            >
              ...
              {p.id === "files" && files.length ? (
                <span className="rounded-[var(--r-tight)] bg-sunk px-1 text-[10px] tabular-nums text-ink-4">{files.length}</span>
              ) : null}
            </button>
          ))}
        </div>
          ...
          {(phase === "ready" || phase === "building") && files.length ? (
            <div className="border-t border-line p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {QUICK_EDITS.map((e) => (
                  <button key={e} type="button" disabled={busy} onClick={() => void edit(e)} className="chip !px-2 !py-1 !text-[11.5px] disabled:opacity-40">
                    {e}
                  </button>
                ))}
              </div>
        ...
        <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col", mobile && half === "chat" && "hidden")}>
          ...
          {pane === "code" ? (
            <div className="min-h-0 flex-1 overflow-auto p-3">
              ...
              <pre className="overflow-auto rounded-[12px] border border-line bg-sunk p-3 font-mono text-[12px] leading-relaxed text-ink-2">
      ...
      {mobile ? (
        <div className="flex border-t border-line">
          <button type="button" onClick={() => setHalf("chat")} className={cn("flex-1 py-2 text-[13px]", half === "chat" ? "text-accent" : "text-ink-4")}>
            Build
          </button>
          <button type="button" onClick={() => setHalf("build")} className={cn("flex-1 py-2 text-[13px]", half === "build" ? "text-accent" : "text-ink-4")}>
            Preview
          </button>
        </div>
      ) : null}
