...
export function BuilderView({ mobile = false, draft = "" }: { mobile?: boolean; draft?: string }) {
  ...
  const ask = useCallback(async (text: string, attach?: Attachment[]) => {
    ...
    try {
      ...
      const m = e instanceof Error ? e.message : "Could not prepare questions."; log(`${m} — planning without them`, "warn"); setQuestions([]); void plan_(text, answers);
    }
  ...
  const runStep = useCallback(async (step: PlanStep, style: string, current: ProjectFile[], position: { index: number; total: number }, attach?: Attachment[]): Promise<ProjectFile[]> => {
    ...
    const res = await fetch("/api/builder/step", { method: "POST", headers: { "Content-Type": "application/json" }, signal: controller.signal, body: JSON.stringify({ step, files: current, idea, style, answers, index: position.index, total: position.total, target: targetId, attachments: strip(attach) }) });
    ...
    void (async () => {
      ...
      try {
        ...
        if (res.ok && data?.previewUrl) {
          ...
          log(`live preview ${data.previewUrl}`, "ok");
        } else if (data?.needKey) {
          log("sandbox offline — set E2B_API_KEY for live URL (using local preview)", "warn");
        } else if (!res.ok) {
          log(data?.error ?? "sandbox skipped", "warn");
        }
      } catch {
        log("sandbox skipped", "warn");
      } finally {
  ...
  }, [plan, busy, paused, files, runStep, styleBrief, log, target, idea, targetId, answers]);
  ...
  const edit = useCallback(async (text: string, attach?: Attachment[]) => {
    ...
    try {
      const current = await runStep({ id: `edit-${Date.now()}`, title: text.slice(0, 40), detail: `Apply this change to the existing site, touching only the files it affects: ${text}. Keep locked brand answers (name, theme, etc.) unchanged unless the user explicitly changes them.`, skills: [], files: [] }, plan ? styleBrief(plan) : "Keep the existing visual language. Touch only files that need to change.", files, { index: 0, total: 1 }, attach);
      setFiles(current); setPhase("ready");
      // Refresh sandbox with new files when available
      if (sandboxUrl) {
        void (async () => {
          try {
            const res = await fetch("/api/sandbox/create", {
            ...
            const data = await res.json().catch(() => null);
            if (res.ok && data?.previewUrl) setSandboxUrl(data.previewUrl);
          } catch { /* keep old url */ }
        })();
      }
    } catch (e) { const m = e instanceof Error ? e.message : "Edit failed."; setError(m); log(m, "warn"); setPhase("ready"); }
  }, [busy, paused, plan, files, runStep, styleBrief, log, sandboxUrl]);
        ...
        <div className="nx-in mx-auto flex w-full max-w-[760px] flex-1 flex-col justify-center px-5 py-16">
          <div className="mb-8 text-center">
            ...
            <p className="mx-auto mt-2 max-w-[48ch] text-[15px] leading-relaxed text-ink-3">
              Describe the outcome. Trove plans the work, builds working multi-page sites, and opens a live preview automatically.
            </p>
          </div>
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col">
      ...
      {phase === "ready" && files.length ? (
        <div className="border-b border-line bg-rail/40 px-3 py-2">
          ...
          {pane === "preview" ? (
            <div className="flex min-h-0 flex-1 flex-col bg-sunk">
              <div className="flex items-center gap-2 border-b border-line px-3 py-2">
                ...
                {pages.length > 1 ? (
                  <select className="rounded border border-line bg-rail px-2 py-1 text-[12px]" value={entry} onChange={(e) => setPage(e.target.value)}>
              ...
              <div className="flex min-h-0 flex-1 items-start justify-center overflow-auto p-3">
                {preview || sandboxUrl ? (
                  <div
                    className="flex h-full min-h-[480px] flex-col overflow-hidden rounded-[14px] border border-line bg-[#1a1a1c] shadow-[0_24px_80px_-40px_rgba(0,0,0,.8)] transition-[width] duration-200"
                    style={{ width: DEVICE[device].w, maxWidth: "100%" }}
                  >
                    <div className="flex h-10 shrink-0 items-center gap-2 border-b border-white/8 bg-[#222226] px-3">
                      <span className="size-2.5 rounded-full bg-[#ff5f57]" />
                      <span className="size-2.5 rounded-full bg-[#febc2e]" />
                      <span className="size-2.5 rounded-full bg-[#28c840]" />
                      <div className="ml-2 flex min-w-0 flex-1 items-center gap-2 rounded-md bg-black/35 px-2.5 py-1 text-[11px] text-white/55">
                        ...
                        {sandboxBooting ? (
                          <span className="ml-auto shrink-0 text-accent">booting…</span>
                        ) : sandboxUrl ? (
                          <span className="ml-auto shrink-0 text-positive">live</span>
                        ) : (
                          <span className="ml-auto shrink-0 text-white/30">local</span>
                        )}
                      </div>
                    </div>
                    <iframe
                  ...
                  <div className="m-auto text-center">
                    <p className="text-[13px] text-ink-4">{busy ? "Building preview…" : "Preview appears as files are written."}</p>
                    {!busy && files.length === 0 ? (
                      <button type="button" className="mt-3 rounded-full border border-line px-3 py-1.5 text-[12.5px] text-ink-3 hover:bg-hover" onClick={() => setPane("console")}>
                        Open console
                      </button>
                    ) : null}
                  </div>
          ...
          {pane === "console" ? (
            <div className="flex min-h-0 flex-1 flex-col">
              ...
              <ProjectTerminal
                ...
                onPreviewUrl={(url) => setSandboxUrl(url)}
              />
            </div>
          ) : null}
