    ...
    <div className="relative flex min-h-screen flex-col">
      ...
      <div className="nx-in mx-auto flex w-full max-w-[760px] flex-1 flex-col justify-center px-5 py-16">
        <div className="mb-8 text-center">
          ...
          <p className="mx-auto mt-2 max-w-[48ch] text-[15px] leading-relaxed text-ink-3">Describe the outcome. Trove plans the work, builds working interactive sites, and keeps them editable.</p>
        </div>
  ...
  return (
    <div className="flex h-screen flex-col">
      ...
      <header className="flex shrink-0 items-center gap-2 border-b border-line bg-rail/70 px-3 py-2 backdrop-blur-xl">
        ...
        <button type="button" onClick={reset} className="chip group !px-2.5 !py-1.5 !text-[12.5px]"><Ico icon={FiRotateCcw} motion="spin" size={13} /><span className="hidden lg:inline">Reset</span></button>
        {files.length ? <button type="button" onClick={() => void download()} className="chip group !px-2.5 !py-1.5 !text-[12.5px]"><Ico icon={FiDownload} motion="nudge" size={13} /><span className="hidden lg:inline">Zip</span></button> : null}
      </header>
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <div className={cn("flex min-h-0 flex-col border-b border-line lg:w-[420px] lg:border-b-0 lg:border-r", mobile && half === "build" && "hidden")}> 
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-3">
            ...
            {questions.length && questionsOpen ? <QuestionBox questions={questions} onSubmit={(a) => void plan_(idea, a)} onSkip={() => void plan_(idea, {})} /> : null}
            {plan ? <PlanPanel plan={plan} onBuild={() => void generate()} onEdit={() => setPhase("review")} building={phase === "building"} /> : null}
            {plan ? <StepsBox steps={plan.steps} states={stepStates} current={currentStep} /> : null}
          ...
          {(phase === "ready" || phase === "building") && files.length ? (
            <div className="border-t border-line p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">{QUICK_EDITS.map((e) => <button key={e} type="button" disabled={busy} onClick={() => void edit(e)} className="chip !px-2 !py-1 !text-[11.5px] disabled:opacity-40">{e}</button>)}</div>
            </div>
          ) : null}
        </div>
        ...
        <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col", mobile && half === "chat" && "hidden")}>
          {pane === "preview" ? (
            <div className="flex min-h-0 flex-1 flex-col bg-sunk">
              <div className="flex items-center gap-2 border-b border-line px-3 py-2">
                ...
                {preview ? <button type="button" onClick={openTab} className="grid size-7 place-items-center rounded-[var(--r-chip)] text-ink-4 hover:text-ink" aria-label="Open"><FiExternalLink size={14} /></button> : null}
              </div>
              <div className="flex min-h-0 flex-1 items-start justify-center overflow-auto p-3">
                {preview ? (
                  <iframe
                    title="Site preview"
                    srcDoc={preview}
                    className="h-full min-h-[480px] rounded-[12px] border border-line bg-white shadow-sm transition-[width] duration-200"
          ...
          {target.commands.length && !target.previewable ? <RunPanel commands={target.commands} serves={target.serves} /> : null}
        </div>
      </div>
