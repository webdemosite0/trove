...
export function BuilderView({
  ...
  async function bootSandbox(current: ProjectFile[]) {
    ...
    log("Starting cloud sandbox (npm install + vite)…");
    try {
      ...
      if (res.ok && data?.previewUrl) {
        ...
        log("Set E2B_API_KEY on Vercel (e2b.dev/dashboard) for live React Preview", "warn");
      } else {
        log(data?.error ?? `Sandbox failed (${res.status})`, "warn");
      }
    } catch {
  ...
  const generate = useCallback(async () => {
    ...
    setFinalMsg(`Built ${title} with ${current.length} files. Starting live Preview…`);
    try {
      ...
      <div className="flex min-h-0 flex-1">
        <div className="flex w-full min-w-0 flex-col border-r border-line lg:w-[42%]">
          <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
            ...
            {phase === "review" && questionsOpen && questions.length > 0 ? (
              <QuestionBox
                questions={questions}
                busy={busy}
                onSubmit={(a) => {
            ...
            {(phase === "building" || phase === "ready") && (
              <div className="space-y-2">
                {finalMsg ? <p className="text-[13.5px] text-ink-2">{finalMsg}</p> : null}
                {tasks.map((t) => (
                  <ProcessRow
                    key={t.id}
                    kind={t.state === "ok" ? "ok" : t.state === "fail" ? "cmd" : "think"}
                    label={t.label}
                    active={t.state === "run"}
                  />
                ))}
                {phase === "building" ? <WorkingTimer secs={workSecs} /> : null}
                {sandboxBooting ? <ProcessRow kind="think" label="Starting sandbox (npm install + vite)…" active /> : null}
              ...
              <div className="mt-4">
                <RunPanel
                  ...
                  onDownload={() => {
                    try {
                      const blob = new Blob(
                        [JSON.stringify({ files, title: plan?.title || idea }, null, 2)],
          ...
          <div className="shrink-0 border-t border-line p-3">
            {mobile ? (
              <MobileComposer onSend={ask} placeholder="Refine or rebuild…" />
            ) : (
              <Composer onSend={ask} placeholder="Refine or rebuild…" />
            )}
          </div>
        </div>
          ...
          <div className="flex shrink-0 items-center gap-1 border-b border-line px-2 py-1.5">
            ...
            {sandboxUrl ? (
              <button
                type="button"
                onClick={openTab}
                className="ml-auto flex items-center gap-1 rounded-full px-2 py-1 text-[12px] text-ink-3 hover:bg-hover"
              >
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            {pane === "preview" ? (
              <div className="flex min-h-0 flex-1 flex-col p-3">
                <BrowserFrame url={sandboxUrl || "localhost"} onOpen={sandboxUrl ? openTab : undefined}>
                  {sandboxUrl ? (
                    <iframe
                      title="Preview"
                      src={sandboxUrl}
                    ...
                    <div className="grid h-full place-items-center text-[13px] text-ink-4">
                      {phase === "building" || sandboxBooting
