...
export function DesignView({
  ...
  async function exportZip() {
    ...
    zip.file(
      "README.txt",
      `Trove design export\n${brief ? briefSummary(brief) : ""}\nScreens: ${ready.map((s) => s.name).join(", ")}\n`,
    );
  ...
  function undoScreen() {
    ...
    setScreens((s) =>
      s.map((x) => (x.name === active.name ? { ...x, html: prev, state: "done" } : x)),
    );
    setChat((c) => [
      ...c,
      { id: chatId.current++, role: "model", text: `Undid last change on ${active.name}` },
    ]);
  }
  ...
  async function sendUpdate(raw: string) {
    ...
    const targets = applyAll
      ? screens.filter((s) => s.state === "done" && s.html)
      : [active];
    ...
    for (const target of targets) {
      ...
      setScreens((s) =>
        s.map((x) =>
          x.name === target.name ? { ...x, html: result.html, state: "done" } : x,
        ),
      );
    }
    if (okCount) {
      setChat((c) => [
        ...c,
        {
          ...
          role: "model",
          text: applyAll
            ? `Updated ${okCount} screen${okCount === 1 ? "" : "s"}${errors.length ? ` (${errors.length} failed)` : ""}`
            : `Updated ${active.name}`,
        },
      ]);
    }
    if (errors.length && !okCount) {
      ...
      setChat((c) => [
        ...c,
        { id: chatId.current++, role: "model", text: `Could not apply that: ${errors[0]}` },
      ]);
    }
    setUpdating(false);
  }
  ...
  const fittedZoom =
    availableWidth === null
      ? zoom
      : Math.min(zoom, (availableWidth - (isMobile ? 28 : 8)) / frame.width);
  const done = screens.filter((s) => s.state === "done").length;

  const deviceShell = (child: ReactNode) => {
    if (!isMobile) {
      return (
        <div className="overflow-hidden rounded-[14px] border border-line bg-raised shadow-[var(--sh-2)]">
          {child}
        </div>
      );
    }
    return (
      ...
      >
        <div
          aria-hidden
          className="absolute left-1/2 top-[10px] z-10 h-[18px] w-[88px] -translate-x-1/2 rounded-full bg-ink"
        />
        ...
        {active
          ? deviceShell(
              ...
              >
                {active.state === "done" ? (
                  ...
                  <div className="grid h-full place-items-center bg-sunk/40 text-[13px] text-ink-4">
                    <span className="inline-flex items-center gap-2">
                      <Ico icon={FiLoader} motion="spin" size={16} className="animate-spin" /> Drawing…
                    </span>
                  </div>
                ) : active.state === "failed" ? (
                  <div className="grid h-full place-items-center bg-sunk/40 px-6 text-center text-[13px] text-critical">
                    {active.error ?? "This screen failed."}
                  </div>
                ) : (
                  <div className="grid h-full place-items-center bg-sunk/40 text-[13px] text-ink-4">Waiting…</div>
                )}
                ...
                {pendingPin ? (
                  <span
                    className="pointer-events-none absolute z-20 size-6 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent ring-2 ring-white"
  ...
  const filmstrip = (
    <div ref={stripRef} className="flex gap-2 overflow-x-auto px-1 py-1 scrollbar-none lg:flex-col lg:overflow-x-hidden lg:overflow-y-auto">
      {screens.map((s, i) => {
        ...
        return (
          ...
          >
            <div className="relative overflow-hidden bg-raised" style={{ width: frame.width * thumbZoom, height: frame.height * thumbZoom }}>
              {s.state === "done" ? (
                <iframe
  ...
  const chatPanel = (
    <div className="flex h-full min-h-0 flex-col">
      <div className="shrink-0 border-b border-line px-3 py-2.5">
        ...
        <p className="mt-0.5 text-[11.5px] text-ink-4">Change colors, layout, copy, or components.</p>
        <label className="mt-2 flex cursor-pointer items-center gap-2 text-[12px] text-ink-3">
          <input type="checkbox" checked={applyAll} onChange={(e) => setApplyAll(e.target.checked)} className="rounded border-line accent-[var(--color-accent)]" />
          Apply to all screens
        </label>
      </div>
      <div className="shrink-0 border-b border-line px-2.5 py-2">
        <p className="mb-1.5 px-0.5 text-[10.5px] font-semibold uppercase tracking-[0.08em] text-ink-4">Components</p>
        <div className="flex max-h-[88px] flex-wrap gap-1 overflow-y-auto">
          {COMPONENT_CHIPS.map((chip) => (
            <button
              ...
              onClick={() => void sendUpdate(chip)}
              className="rounded-full border border-line bg-raised px-2 py-0.5 text-[11px] text-ink-3 transition hover:border-accent/40 hover:text-ink disabled:opacity-40"
            >
              {chip}
            </button>
          ))}
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-3 py-3">
        {chat.length === 0 ? (
          <div className="rounded-[14px] border border-dashed border-line bg-sunk/40 px-3 py-4 text-[12.5px] leading-relaxed text-ink-3">
            Type a change, or tap a component above. Toggle <span className="font-medium text-ink-2">Apply to all screens</span> to keep the set consistent.
          </div>
        ) : (
          chat.map((t) => (
            <div
              ...
              className={cn(
                ...
                t.role === "user" ? "ml-auto bg-accent text-white" : "mr-auto border border-line bg-raised text-ink-2",
              )}
            >
              {t.text}
            </div>
        ...
        {updating ? (
          <div className="inline-flex items-center gap-2 text-[12.5px] text-ink-4">
            <Ico icon={FiLoader} motion="spin" size={14} className="animate-spin" /> Updating…
          </div>
        ) : null}
        <div ref={chatEnd} />
      </div>
      <form className="shrink-0 border-t border-line p-2.5" onSubmit={(e) => { e.preventDefault(); void sendUpdate(draft); }}>
        <div className="flex items-end gap-2 rounded-[16px] border border-line bg-sunk px-2.5 py-2 focus-within:border-accent/40">
          <textarea
            ...
            rows={2}
            disabled={busy || updating || active?.state !== "done"}
            placeholder={active?.state === "done" ? (applyAll ? "Update all screens…" : `Update ${active.name}…`) : "Wait for the screen to finish…"}
            className="max-h-28 min-h-[40px] flex-1 resize-none bg-transparent text-[13.5px] text-ink outline-none placeholder:text-ink-4"
            onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void sendUpdate(draft); } }}
          />
          <button type="submit" disabled={!draft.trim() || busy || updating || active?.state !== "done"} className="grid h-9 w-9 shrink-0 place-items-center rounded-full btn-grad text-white disabled:opacity-40" aria-label="Send update">
            <FiArrowUp size={15} />
          </button>
        </div>
      </form>
  ...
  const commentsPanel = (
    <div className="flex h-full min-h-0 flex-col">
      <div className="shrink-0 border-b border-line px-3 py-2.5">
        ...
        <p className="mt-0.5 text-[11.5px] text-ink-4">Pin notes on the current screen like Figma.</p>
        <button
          type="button"
          onClick={() => { setPinMode((v) => !v); setPendingPin(null); }}
          className={cn(
            "mt-2 w-full rounded-[10px] border px-3 py-1.5 text-[12.5px] font-medium transition",
            pinMode ? "border-violet-500 bg-violet-500 text-white" : "border-line bg-raised text-ink-2 hover:border-line-strong",
          )}
        >
          {pinMode ? "Click the preview to place a pin…" : "Add pin on preview"}
        </button>
      </div>
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto px-3 py-3">
        {pendingPin ? (
          <div className="rounded-[14px] border border-accent/40 bg-accent/5 p-3">
            <p className="text-[12px] font-medium text-ink">New pin</p>
            <textarea value={commentDraft} onChange={(e) => setCommentDraft(e.target.value)} rows={2} placeholder="What should change here?" className="mt-2 w-full resize-none rounded-[10px] border border-line bg-sunk px-2.5 py-2 text-[13px] text-ink outline-none focus:border-accent/40" autoFocus />
            <div className="mt-2 flex gap-2">
              <button type="button" onClick={addComment} disabled={!commentDraft.trim()} className="rounded-[10px] btn-grad px-3 py-1.5 text-[12.5px] font-medium text-white disabled:opacity-40">Save pin</button>
              <button type="button" onClick={() => { setPendingPin(null); setCommentDraft(""); }} className="rounded-[10px] border border-line px-3 py-1.5 text-[12.5px] text-ink-3">Cancel</button>
            </div>
          </div>
        ) : null}
        {screenComments.length === 0 && !pendingPin ? (
          <p className="text-[12.5px] text-ink-4">No comments on this screen yet. Turn on pin mode and click the preview.</p>
        ) : (
          screenComments.map((c, i) => (
            <div key={c.id} className="rounded-[12px] border border-line bg-raised px-3 py-2.5">
              <div className="flex items-start gap-2">
                <span className="grid size-5 shrink-0 place-items-center rounded-full bg-violet-500 text-[10px] font-bold text-white">{i + 1}</span>
                <p className="min-w-0 flex-1 text-[13px] leading-relaxed text-ink-2">{c.text}</p>
                <button type="button" aria-label="Remove comment" onClick={() => setComments((all) => all.filter((x) => x.id !== c.id))} className="text-ink-4 hover:text-critical"><FiX size={14} /></button>
              </div>
              <button type="button" className="mt-2 text-[11.5px] font-medium text-accent hover:underline" onClick={() => { setMobilePane("chat"); setDraft(`Regarding pin ${i + 1}: ${c.text}`); }}>
                Send to update chat →
              </button>
            </div>
          ))
  ...
  return (
    <div className="mobile-editor design-editor flex h-full min-h-0 flex-col overflow-hidden">
      <header className="flex shrink-0 flex-wrap items-center gap-2 border-b border-line bg-canvas/90 px-3 py-2.5 backdrop-blur-md sm:gap-3 lg:px-5">
        <div className="min-w-0 flex-1">
          <p className="text-[11.5px] uppercase tracking-[0.08em] text-ink-4">
            Design · {done} of {screens.length}{busy ? " · drawing…" : ""}
          </p>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <label className="hidden items-center gap-2 rounded-[var(--r-control)] border border-line bg-rail px-2.5 py-1.5 lg:flex">
            ...
            <input type="range" min={0.35} max={1} step={0.05} value={zoom} onChange={(e) => setZoom(Number(e.target.value))} aria-label="Zoom the screens" className="w-[86px] accent-[var(--color-accent)]" />
            <span className="w-[34px] text-right text-[11.5px] tabular-nums text-ink-4">{Math.round(fittedZoom * 100)}%</span>
          </label>
          <button type="button" onClick={undoScreen} disabled={!activeHistory.length || updating} className="chip group !px-3 !py-1.5 !text-[12.5px] disabled:opacity-30" title="Undo last change on this screen">
            <Ico icon={FiRotateCcw} motion="spin" size={13} /> Undo
          </button>
          {active?.state === "done" ? (
            <>
              <button onClick={() => openScreen(active)} className="chip group !px-3 !py-1.5 !text-[12.5px]"><Ico icon={FiExternalLink} motion="launch" size={13} /> Open</button>
              <button onClick={() => downloadScreen(active)} className="chip group !px-3 !py-1.5 !text-[12.5px]"><Ico icon={FiDownload} motion="down" size={13} /> HTML</button>
            </>
          ) : null}
          <button type="button" onClick={() => void exportZip()} disabled={!done} className="chip group !px-3 !py-1.5 !text-[12.5px] disabled:opacity-30" title="Download all screens as ZIP">
            <Ico icon={FiDownload} motion="down" size={13} /> ZIP
          </button>
          <button type="button" onClick={() => void shareLink()} className="chip group !px-3 !py-1.5 !text-[12.5px]">
            <Ico icon={FiShare2} motion="lift" size={13} /> Share
          </button>
          <button onClick={reset} className="chip group !px-3 !py-1.5 !text-[12.5px]"><Ico icon={FiRotateCcw} motion="spin" size={13} /> New</button>
        </div>
      </header>

      {shareNote ? (
        <div className="shrink-0 border-b border-line bg-accent/10 px-4 py-1.5 text-center text-[12.5px] text-ink-2">{shareNote}</div>
      ) : null}
      ...
      <div className="flex shrink-0 gap-1 border-b border-line px-3 py-2 lg:hidden">
        {([ ["preview", "Screens"], ["chat", "Update"], ["comments", "Pins"] ] as const).map(([id, label]) => (
          <button key={id} type="button" onClick={() => setMobilePane(id)} className={cn("flex-1 rounded-full px-3 py-1.5 text-[12.5px] font-medium transition", mobilePane === id ? "bg-accent text-white" : "bg-sunk text-ink-3 hover:text-ink")}>
            {label}
          </button>
        ))}
      </div>
      ...
      {error && !busy && !updating ? (
        <div className="shrink-0 px-4 pt-3 lg:px-5"><FailureNote error={error} className="w-full" /></div>
      ) : null}
      ...
      <div className="hidden min-h-0 flex-1 lg:grid lg:grid-cols-[112px_minmax(0,1fr)_340px]">
        ...
        <aside className="flex min-h-0 flex-col border-l border-line bg-rail/30">
          <div className="min-h-0 flex-[3] overflow-hidden border-b border-line">{chatPanel}</div>
          <div className="min-h-0 flex-[2] overflow-hidden">{commentsPanel}</div>
        </aside>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:hidden">
        {mobilePane === "preview" ? (
          <>
            ...
            <div className="shrink-0 border-t border-line bg-rail/50 px-3 py-2">{filmstrip}</div>
          </>
        ) : mobilePane === "chat" ? (
