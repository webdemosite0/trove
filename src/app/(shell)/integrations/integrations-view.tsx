    ...
    <div className="min-h-[calc(100dvh-3.5rem)] bg-transparent text-ink">
      <div className="mx-auto max-w-[920px] px-5 pb-16 pt-10 sm:px-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <label className="relative block w-full sm:max-w-[260px]">
            ...
            <input
              ...
              className="h-11 w-full rounded-full border border-line bg-raised/80 pl-10 pr-4 text-[14px] text-ink outline-none placeholder:text-ink-4 focus:border-accent/50 focus:bg-raised"
            />
          </label>
        </div>

        {nangoError ? (
          <p className="mt-4 rounded-xl border border-critical/30 bg-critical-soft px-3.5 py-2.5 text-[13px] text-critical">
            {nangoError}
          </p>
        ) : null}
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              {installed.map((s) => (
                <Link
                  ...
                  title={s.name}
                  className="group relative grid size-[52px] place-items-center rounded-2xl bg-raised shadow-[var(--sh-2)] ring-1 ring-line transition hover:scale-105 hover:ring-line-strong"
                >
        ...
        <section className="mt-12">
          <h2 className="text-[13px] font-medium text-ink-4">Popular</h2>
                ...
                <li key={s.id}>
                  <div className="group flex items-center gap-3.5 rounded-2xl px-2.5 py-3 transition hover:bg-hover/70">
                    ...
                    <Link href={`/integrations/${s.id}`} className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-[13px] text-ink-4">{s.blurb}</p>
                    </Link>
                    {on ? (
                      <button
                        type="button"
                        disabled={pending}
                        onClick={() => remove(s.id)}
                        className="grid size-9 shrink-0 place-items-center rounded-full border border-positive/35 bg-positive-soft text-positive"
                      ...
                      <button
                        ...
                        className="grid size-9 shrink-0 place-items-center rounded-full border border-line-strong text-ink-3 transition hover:border-accent/45 hover:bg-hover hover:text-ink disabled:opacity-40"
