...
export default async function BillingHistoryPage() {
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="rounded-[22px] border border-line bg-raised shadow-[var(--sh-1)]">
        <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          ...
          {hasBillingProfile ? (
            ...
            <a href="/plans" className="rounded-xl border border-line-strong bg-raised px-4 py-2.5 text-center text-[12.5px] font-semibold text-ink transition hover:bg-hover">
              View plans
            </a>
          )}
        </div>
        <div className="border-t border-line bg-sunk/70 px-5 py-4 sm:px-6">
      ...
      <div className="rounded-[20px] border border-line bg-raised p-5 shadow-[var(--sh-1)] sm:p-6">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          {["Download receipts", "View invoices", "Update tax details", "Check payment status", "Change payment method", "Manage renewal or cancellation"].map((item) => (
            <div key={item} className="flex items-center gap-2.5 rounded-xl border border-line/70 bg-sunk/70 px-3.5 py-3 text-[12.5px] text-ink-2">
