    ...
    <div className="relative mx-auto min-h-screen max-w-[1100px] px-5 py-10 lg:px-8">
      ...
      {checkout === "cancelled" ? (
        <div className="nx-in mb-6 flex items-center gap-2.5 rounded-[var(--r-panel)] border border-line bg-rail px-4 py-3">
          ...
          <p className="text-[13.5px] text-ink-2">Checkout was cancelled. Nothing was charged.</p>
        </div>
      ) : null}
      ...
      {checkout === "done" && waiting ? (
        <div className="nx-in mb-6 flex items-center gap-3 rounded-[var(--r-panel)] border border-accent/30 bg-accent-soft px-4 py-3">
          <Ico icon={FiLoader} motion="spin" size={15} className="shrink-0 animate-spin text-accent" />
      ...
      {checkout === "done" && !waiting && !paidNow ? (
        <FailureNote
          className="mb-6"
          error="Your payment went through, but the plan has not switched over yet. Confirmation can take a few seconds. Refresh shortly — or open Manage billing if it stays on the old plan."
        />
      ) : null}
        ...
        <section className="panel nx-in mb-5 p-6">
          ...
          {usage.length ? (
            <ul className="mt-5 space-y-2 border-t border-line pt-4">
              {usage.map((u) => (
                <li key={u.kind} className="flex items-center justify-between gap-3 text-[13.5px]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="flex items-center gap-2 text-[13.5px] font-medium text-ink">
                Subscription
                {status ? (
                  <span className={cn("inline-flex items-center gap-1.5 text-[12.5px]", status.tone)}>
  ...
  if (!buyable) {
    return (
      <span
        ...
        title={
          stripeReady
            ? `No price is configured for ${name} on this deployment.`
